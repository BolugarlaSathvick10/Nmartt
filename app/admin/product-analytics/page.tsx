...
export default function ProductAnalyticsPage() {
  ...
  const productActivities = useCatalogStore((state) => state.productActivities);
  ...
  return (
    <div className="w-full flex flex-col gap-6">
              ...
              <div className="space-y-3">
                <div className="flex justify-between py-2 border-b border-gray-200">
                  ...
                  <span className="text-sm font-medium text-gray-900">
                    {localizeProductName(selected.name, locale)}
                  </span>
                </div>
                <div className="flex justify-between py-2 border-b border-gray-200">
                  <span className="text-sm text-gray-600">{t("adminAnalytics.category")}</span>
                  <span className="text-sm font-medium text-gray-900">
                    {localizeCategoryName(selected.categoryName, locale)}
                  </span>
                </div>
                <div className="flex justify-between py-2 border-b border-gray-200">
              ...
              {results.map((r) => (
                ...
                >
                  <div className="text-sm font-medium text-gray-900">
                    {localizeProductName(r.name, locale)}
                  </div>
                  <div className="text-xs text-gray-500">{localizeCategoryName(r.categoryName, locale)}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      <Card className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <div className="space-y-4">
          <h3 className="text-lg font-medium text-gray-900">Recent Product Activity</h3>
            ...
            {productActivities.slice(0, 12).map((activity) => (
              <div key={activity.id} className="rounded-lg border border-gray-100 p-3 text-sm">
                <p className="font-medium text-gray-900">
                  {activity.action.replace("_", " ").toUpperCase()} - {localizeProductName(activity.productName, locale)}
                </p>
                <p className="text-gray-500">
                  {activity.actorName ?? "Unknown"} ({activity.actorRole ?? "unknown"}) | {new Date(activity.timestamp).toLocaleString()}
                </p>
              </div>
            ))}
